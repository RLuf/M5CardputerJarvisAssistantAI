...
void displayText(const String &text, int &scrollPosition, int scrollDirection, bool isSecondInput) {
  ...
  int textAreaHeight = (M5Cardputer.Display.height()  3) + 3;   Use bottom 13 of the screen
  int textAreaStartY = M5Cardputer.Display.height() - textAreaHeight;

  if (isSecondInput) {

     Draw a rectangle to mask the bottom 13 of the screen
    ...
    lineHeight = canvas.fontHeight();
     ...
     Calculate the number of displayable lines for the bottom 13 of the screen
    ...
    for (size_t i = scrollPositionDOWN; i  scrollPositionDOWN + numDisplayableLines && i  numLines; i++) {
     ...
     Push the canvas content to the display, offsetting the starting position to the bottom 13 of the screen
...
void loadConfig() {
  ...
  if (configFile) {
    ...
    while (configFile.available()) {
      line = configFile.readStringUntil('n');
...
void handleMenuSelection() {
  ...
  M5Cardputer.Display.setTextFont(&fontsFreeMono9pt7b);
  ...
  if (M5Cardputer.Keyboard.isChange()) {
    ...
    if (M5Cardputer.Keyboard.isPressed()) {
      ...
      Keyboard_ClassKeysState status = M5Cardputer.Keyboard.keysState();
...
M5Cardputer.Display.setCursor(0, 0);
      M5Cardputer.Display.setTextColor(RED);
M5Cardputer.Display.pushImage(0, 0, 240, 135, error);
      M5Cardputer.Display.println(nFailed to connect to Wi-Fi, switching to menu...);
      delay(2000);
      M5Cardputer.Display.fillScreen(BLACK);
      Menu = true;
      ConfigMenu();
      ...
      M5Cardputer.Display.println(nRestarting...);
      delay(2000);
      ESP.restart();
    }
  }

  M5Cardputer.Display.println(nWi-Fi connected);
  M5Cardputer.Display.println(nIP  + WiFi.localIP().toString());
  delay(1000);
  M5Cardputer.Display.fillScreen(BLACK);
}
  ...
  if (M5Cardputer.Keyboard.isChange()) {
    if (M5Cardputer.Keyboard.isPressed()) {
      ...
      if (status.enter) {
        ...
        M5Cardputer.Display.setTextFont(&fontsOrbitron_Light_24);
...
bool sendGeminiRequest(const String &input) {
  ...
  String url = httpsgenerativelanguage.googleapis.comv1betamodelsgemini-progenerateContentkey= + String(api_key);
  ...
  M5Cardputer.Display.setTextFont(&fontsFreeMono9pt7b);
    ...
    M5Cardputer.Display.fillScreen(BLACK);
    M5Cardputer.Display.setTextColor(RED);
    M5Cardputer.Display.println(nHTTP error  + String(httpResponseCode));
...
void processGeminiResponse(const String &response) {
  ...
  geminiResponse = responseDoc[candidates][0][content][parts][0][text].asString();
